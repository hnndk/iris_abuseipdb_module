...
class IrisAbuseipdbInterface(IrisModuleInterface):
    ...
    def _handle_ioc(self, data) -> InterfaceStatus.IIStatus:
        ...
        for element in data:
            ...
            self.log.info(f'Processing IOC type: {ioc_type} with value: {element.ioc_value}')
            ...
            # Verifica se é um IP (ip-src, ip-dst, ipv4, ipv6, etc)
            elif 'ip' in ioc_type:
                status = abuseipdb_handler.handle_ip(ioc=element)
                in_status = InterfaceStatus.merge_status(in_status, status)
            else:
                self.log.error(f'IOC type {ioc_type} not handled by abuseipdb module. Skipping')
