...
class AbuseipdbHandler(object):
    def __init__(self, mod_config, server_config, logger):
        ...
        self.abuseipdb = self.get_abuseipdb_instance()
        self.log = logger

    def get_abuseipdb_instance(self):
        """
        Returns an abuseipdb API instance depending if the key is premium or not

        :return: { cookiecutter.keyword }} Instance
        """
        url = self.mod_config.get('abuseipdb_url')
        key = self.mod_config.get('abuseipdb_key')
        proxies = {}
        ...
        # TODO!
        # Here get your abuseipdb instance and return it
        # ex: return abuseipdbApi(url, key)
        return "<TODO>"

    def gen_domain_report_from_template(self, html_template, abuseipdb_report) -> InterfaceStatus:
        """
        ...
        :param misp_report: The JSON report fetched with abuseipdb API
        ...
        for abuseipdb_result in context:
            pre_render["results"].append(abuseipdb_result)
    ...
    def handle_domain(self, ioc):
        ...
        if self.mod_config.get('abuseipdb_report_as_attribute') is True:
            ...
            report = ["<TODO> report from abuseipdb"]

            status = self.gen_domain_report_from_template(self.mod_config.get('abuseipdb_domain_report_template'), report)
            ...
            try:
                add_tab_attribute_field(ioc, tab_name='abuseipdb Report', field_name="HTML report", field_type="html",
